(function ($) {
	$(function () {
		...
		$(window).on('scroll', function() {
			($(this).scrollTop() && !$global.header.is('scrolled')) ? 
		...
		function expandMapFront() {
			let $map = $('#map-front');
			let posPage = $(document).scrollTop();
			$map.toggleClass('expand');
			($map.is('expand')) ? 
				scrollPage(posPage - 300, 260) : 
				scrollPage(posPage + 300, 260);
		}
		...
		(function sliderFrontInit() {
			const $slider = $global.sliderFront;
			const controls = getControls($slider);
		...
		(function servicesFrontInit() {
			const $slider = $global.servicesFront;
			const controls = getControls($slider);
		...
		(function projectsFrontInit() {
			const $slider = $global.projectsFront;
			const controls = getControls($slider);
		...
		(function clientsFrontInit() {
			const $slider = $global.clientsFront;
			const controls = getControls($slider);
		...
		function getControls($slider) {
			const prevCls = '.slider__arrow_prev';
			const nextCls = '.slider__arrow_next';
...
function initMap() {
	let myLatLng = {lat: 53.8668267, lng: 27.5078881};
  let map = new google.maps.Map(document.getElementById('map-front'), {
    center: myLatLng,
    zoom: 15,
    scrollwheel: false
  });
